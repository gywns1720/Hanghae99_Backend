...
import {
  ChargePointCommand,
  SubtractPointCommand,
} from '@lib/e-commerce/user/command';
...
export class SubtractPointUseCase {
  ...
  async execute(data: CommonPointCaseDto | SubtractPointCommand) {
    if (this._isPointCommand(data)) {
      await this.retryPointCharge(data);
    } else {
      await this._subtractPointDto(data);
    }
  }

  private async _subtractPointCommand(data: SubtractPointCommand) {
    await this.retryPointCharge(data);
   ...
   * @throws UserError
   */
  private async _subtractPointDto(data: CommonPointCaseDto) {
  ...
  private _isPointCommand(
    item: CommonPointCaseDto | SubtractPointCommand,
  ): item is SubtractPointCommand {
    return item instanceof SubtractPointCommand;
  }

  // TODO 부모 Point 만들어서 production 으로 집어넣으면 어떨까
  private async retryPointCharge(
    data: CommonPointCaseDto | SubtractPointCommand,
  ) {
    const maxAttempts = 4;
    const delayMs = 500;
    ...
    for (let attempt = 1; attempt <= maxAttempts; attempt++) {
      try {
        await this.userRepo.transaction(async (manager) => {
          ...
          if (user.point - saveData.point < 0) {
            throw UserError.LackOfPoint();
          }
